...
model_z_mean = Sequential([
    Dense(20, input_dim=5, activation=tf.nn.leaky_relu),
    Dense(15, activation=tf.nn.leaky_relu),
    Dense(1, activation='sigmoid'),
    ])
model_z_mean.compile(optimizer = 'adam', loss='binary_crossentropy', metrics=[tf.keras.metrics.AUC(name='auc')])

name = 'cvae_latent'
file_name = name + datetime.now().strftime('%Y%m%d')

model_z_mean_history = model_z_mean.fit(
    z_mean_train, y_train[:,1],
    validation_data= (z_mean_val, y_val[:,1]),
    callbacks = [
        tf.keras.callbacks.EarlyStopping(
...
# ## Plot loss
# # Check plots - Loss
plt.plot(model_z_mean_history.history['loss'])
plt.plot(model_z_mean_history.history['val_loss'])
plt.title('model loss')
plt.ylabel('loss')
plt.xlabel('epoch')
plt.legend(['train', 'val'], loc='upper right');

fpr, tpr, thresholds = metrics.roc_curve(np.array(y_test[:,1]), model_z_mean.predict(z_mean_test)[:,0], pos_label=1)
metrics.auc(fpr, tpr)

df_pf_cvae_clf = performance_rank_df(y_test[:,1], model_z_mean.predict(z_mean_test)[:,0], if_score = False)
performance_rank_n(df_pf_cvae_clf)
